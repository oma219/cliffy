...
MinimizerDigest::MinimizerDigest(uint64_t k, uint64_t w): k(k), w(w), loaded_kmers(0) {
...
std::string MinimizerDigest::compute_digest(std::string input_seq) {
    ...
    auto load_first_kmer_to_queue = [&] () {
        ...
        MinimizerData curr_kmer_data {(str_pos-k), curr_kmer_val};
        queue.push_back(curr_kmer_data);
        loaded_kmers++;
        //std::cout << (str_pos-k) << " "  << curr_kmer_val << " " << input_seq.substr((str_pos-k), k) << std::endl;
    };
    ...
    auto append_kmer_to_queue = [&] (uint64_t new_pos, uint64_t new_val) {
        ...
        MinimizerData new_kmer {new_pos, new_val};
        queue.push_back(new_kmer);
        loaded_kmers++;

        //std::cout << new_pos << " "  << new_val << " " << input_seq.substr(new_pos, k) << std::endl;
    };
    ...
    while (str_pos <= (input_seq.size()-1)) {
        ...
        if (loaded_kmers >= (w-k+1) && queue[0].val != last_minimizer) { 
            //std::cout << "MINIMIZER FOUND: pos = " << queue[0].pos << ", val = " << queue[0].val << ", string = " << input_seq.substr(queue[0].pos,k) << std::endl;
